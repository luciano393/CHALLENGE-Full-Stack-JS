...
export const Balance = ({trans}) => {
    let Gastos = 0;
    let Ingresos = 0;
    let sumaGastos = 0;
    let sumaIngresos = 0;
    
    const arrayGastos = trans.map(trans => ((trans.tipo === 'gasto') && Gastos + Math.floor(trans.monto)))
    const arrayIngresos = trans.map(trans => ((trans.tipo === 'ingreso') && Ingresos + Math.floor(trans.monto)))
